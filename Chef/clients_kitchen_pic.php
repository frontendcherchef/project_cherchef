...
//算總比數
$t_sql = "SELECT COUNT(1) FROM clients_kitchen_pics";
      ...
      <td>
            ...
            foreach ($all_pics as $pic) :
                if ($row['sid'] == $pic['clients_sid']) : ?>
                    <a href="/mytest/Chef_pic/kitchen_photo/<?= $pic['file_name'] ?>" data-lightbox="roadtrip+<?=$pic['clients_sid']?>"><i class="fas fa-images"></i></a>
                <?php endif;
            endforeach
            ?>
      </td>
...
<script>
        function delete_it(sid, file_name){
            if(confirm(`確定要刪除編號為 ${sid} 的資料嗎?`)){
                location.href = 'clients_kitchen_pic_delete.php?sid=' + sid +'&file_name=' +file_name;
            }
        }
    </script>
